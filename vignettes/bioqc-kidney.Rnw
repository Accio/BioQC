...
\SweaveOpts{keep.source=TRUE,eps=FALSE,pdf=TRUE,prefix=TRUE,eval=FALSE} 
...
First we load BioQC library and the expression data into the R session. 

<<dataReadIn>>=
library(BioQC)
file <- system.file("extdata/bioqc-nephrectomy.RData", package="BioQC")
load(file)
@ 
